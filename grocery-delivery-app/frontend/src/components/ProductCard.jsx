  ...
  return (
    <div className="bg-white rounded-xl border border-kraft-300 overflow-hidden shadow-xs hover:shadow-md transition-shadow flex flex-col justify-between">
      <div>
        ...
        <div className="p-4">
          <h3 className="font-display font-semibold text-lg text-ink-900 line-clamp-1">{product.name}</h3>
          <p className="text-xs text-ink-600 mt-1 line-clamp-2">{product.description}</p>

          <div className="mt-3 flex items-center justify-between">
            <div>
              <span className="text-xl font-bold text-leaf-700">₹{product.price}</span>
            ...
            {product.rating > 0 && (
              <div className="flex items-center text-xs text-turmeric-500 font-medium">
                ...
                <span className="text-ink-600 ml-1">({product.numReviews})</span>
              </div>
            )}
          </div>
        </div>
